    ...
    for i in list:
        ...
        if(key & 0xFF == ord('q')):                                             # if q is pressed
            ...
            if(obj_count > 0):                                                  # and obj_count > 0
                ...
                obj_count = 0
                obj_list = []
                #agregue esto para que me recorte la imagen, pero primero
                #abri nuevamente la foto como frame2, para que no tenga marcas
                #ya que si se con el mismo frame queda marcado el rectangulo
                cropped = frame2[y1:y1+h, x1:x1+w]
                #obtengo el nombre de la imagen para poder guardarla
                name =i.partition("/")
                name = name[2][:-4]
                #print("name: ",name)
                #escribo la imagen recortada en una carpeta llamada recortada
                cv2.imwrite('Recortada/'+name+'_cropped.jpg',cropped)
